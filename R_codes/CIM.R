...
## a
str(sleep)
Y <- sleep[which(sleep$group == 1), 1]
X <- sleep[which(sleep$group == 2), 1]
D <- Y - X

## 4b n4(n_diff), StE(Std_Err)
n_diff <- length(D)
mean.D <- mean(D)
cov(X, Y)
var.D <- var(X) + var(Y) - 2 * cov(X, Y)
Std_Err <- sqrt(var.D) / sqrt(n_diff)

# CI
LCI <- mean.D - 1.96 * (Std_Err)
UCI <- mean.D + 1.96 * (Std_Err)
cbind(LCI, UCI)

## 4c (classical method, non-parametric BS percentile interval, non-parametric BS t-interval)
B <- 10000
store <- c(1:B)
for (i in 1:B) {
  boot.samp <- sample(D, n_diff, replace = T)
  store[i] <- mean(boot.samp)
}

Std_Err1 <- sqrt(var(store))
### CI
...
# read the data
beatle <- read.table("beetle.txt",
  header = FALSE, na.strings = ".", dec = ".",
